...
- Review-packet, token-invariant, claim-ledger, and fork-migration language belongs in config plus `src/composition.py`; do not hide those obligations in generated output or freeform docs only.
- Change `src/config.py` only when the schema changes, and cover new validation behavior in `tests/test_config.py`.
- Change `src/composition.py` when the generated manuscript body or Markdown evidence tables change.
